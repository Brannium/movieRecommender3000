...
def rating_to_label_strength(r):
    if r == -1:
...
labels = []
weights = []
for r in interactions_df["rating"]:
    y, w = rating_to_label_strength(r)
    labels.append(y)
    weights.append(w)

interactions_df["label"] = labels
interactions_df["sample_weight"] = weights

# Drop rows where label is None (rating == 0)
interactions_df = interactions_df[interactions_df["label"].notnull()].reset_index(drop=True)
interactions_df["label"] = interactions_df["label"].astype(np.float32)
interactions_df["sample_weight"] = interactions_df["sample_weight"].astype(np.float32)
...
def tags_to_ids(tag_list):
    ids = [tag2idx.get(t, 0) for t in tag_list][:MAX_TAGS_PER_MOVIE]
    if len(ids) < MAX_TAGS_PER_MOVIE:
        ids += [0] * (MAX_TAGS_PER_MOVIE - len(ids))
    return ids

movies_df["tag_ids"] = movies_df["tags"].apply(tags_to_ids)
...
user_pos_movies = (
    interactions_df[interactions_df["label"] == 1.0]
    .groupby("user_id")["movie_id"]
    .apply(list)
    .to_dict()
)

MAX_USER_MOVIES = 20  # how many liked movies we use per user

class InteractionDataset(Dataset):
    def __init__(self, df, movieid2tags, user_pos_movies,
                 max_user_movies=MAX_USER_MOVIES):
    ...
    def _user_tag_bag(self, user_id, target_movie_id):
        ...
        if not pos_movies:
            # cold user: all PAD
            return [0] * (MAX_TAGS_PER_MOVIE * self.max_user_movies)
        ...
        for mid in pos_movies:
            tag_ids.extend(self.movieid2tags.get(mid, [0]*MAX_TAGS_PER_MOVIE))

        max_len = MAX_TAGS_PER_MOVIE * self.max_user_movies
...
dataset = InteractionDataset(interactions_df, movieid2tags, user_pos_movies)
...
model = TwoTowerRec(vocab_size, emb_dim, emb_matrix).to(device)
...
for epoch in range(EPOCHS):
    ...
    for user_tags, movie_tags, labels, weights in dataloader:
        ...
        optimizer.zero_grad()
        logits = model(user_tags, movie_tags)              # [B]
...
# model.eval()
# 
# movie_tag_tensor = torch.tensor(
#     np.stack(movies_df["tag_ids"].values),
#     dtype=torch.long,
# ).to(device)      # [N_movies, T]
# 
# with torch.no_grad():
#     item_vecs = model.item_encoder(movie_tag_tensor)  # [N_movies, D]
# 
...
with torch.no_grad():
    for i in range(0, len(movies_df), batch_size_inference):
        ...
        batch_vecs = model.item_encoder(batch_tensor)
...
def build_user_tag_bag_for_user(user_id, max_user_movies=MAX_USER_MOVIES):
    pos_movies = (
        interactions_df[
            (interactions_df["user_id"] == user_id) &
            (interactions_df["label"] == 1.0)
        ]["movie_id"]
        .tolist()
    )
...
def recommend_for_user(user_id, top_k=20, exclude_seen=True):
    user_tag_ids = build_user_tag_bag_for_user(user_id)
    user_tag_ids = torch.tensor(user_tag_ids, dtype=torch.long).unsqueeze(0).to(device)

    with torch.no_grad():
        u_vec = model.user_encoder(user_tag_ids)           # [1, D]
        item_vecs_device = item_vecs.to(device)            # Move item_vecs to device
    ...
    if exclude_seen:
        seen_movies = set(
            interactions_df[interactions_df["user_id"] == user_id]["movie_id"].tolist()
        )
...
# Example:
user_id_example = 1
recommendations = recommend_for_user(user_id_example, top_k=30)

# read in full database to find movie names
all_movies = pd.read_csv("../dataset/TMDB_movie_dataset_v11.csv")

for movie_id, score in recommendations:
    movie_row = all_movies[all_movies["id"] == movie_id]
...
watched_movies = interactions_df[interactions_df["user_id"] == user_id_example]
watched_movies = watched_movies.merge(all_movies, left_on="movie_id", right_on="id", how="left")
